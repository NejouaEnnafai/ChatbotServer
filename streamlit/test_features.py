...
def query_model(payload, for_analysis=False):
    """
    Envoie une requête au modèle Llama avec le token Hugging Face
    """
    try:
        inputs = tokenizer(payload, return_tensors="pt").to(model.device)
        with torch.no_grad():
        ...
        # Extraire uniquement la réponse générée (après l'entrée)
        response = response[len(payload):].strip()
        return response
...
def analyze_results(results, question):
    ...
    try:
        ...
        # Appeler le modèle pour l'analyse
